...
const clearItemCart = (cartItems, productIdToRemove) => cartItems.filter((item) => item.id !== productIdToRemove)
...
export const CartProvider = ({children}) => {
  ...
  useEffect(() => {
    const newTotalItems = cartItems.reduce((previousValue, currentValue) => previousValue + currentValue.quantity, 0)
    setTotalItems(newTotalItems)
  }, [cartItems]);

  useEffect(() => {
    const newTotalPrice = cartItems.reduce((previousValue, currentValue) => previousValue + (currentValue.price * currentValue.quantity), 0)
